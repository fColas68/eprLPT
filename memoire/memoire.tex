...
Le problème Bin-packing, qui est NP-Complet, tente de 
  minimiser le nombre de bacs $k$. 
  
Dans la mesure où 
\begin{itemize}
\item l'ensemble des objets à ranger et leur tailles peuvent être vues comme
  un ensemble $J=\{j_1, j_2, \ldots, j_n\}$ de $n$ jobs et les temps de traitement de chacun,
\item la partition $B < B_1, B_2, ... B_m > $ peut s'apparenter à 
  l’ordonnancement recherché sur $m$ machines.
\end{itemize}  
nous 
  
\bigskip
Coffman \emph{et al.} \cite{coffman1978application} utilisent le problème Bin-packing 
  pour essayer de résoudre \problemGrahamP, en utilisant FFD (First Fit Decreasing, algorithme \ref{algo:FFD}), qui est un outil de résolution du problème de packing. 
...
L'ensemble $J=\{j_1, j_2, \ldots, j_n\}$ de $n$ jobs et 
