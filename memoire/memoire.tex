...
% =======================================================
\section{Implémentation des heuristiques}
\label{sec:Implementation}
\fco{a inclure dans Campagnes}
L'objectif est de reproduire le protocole expérimental de 
  \dcs \cite{della2020longest} et aussi de l'élargir à d'autres types d'instances.
  
pour cela, des heuristiques sont implémentées, 
...
Les résultats sont ensuite analysés selon la méthode \dcs mais aussi avec d'autres méthodes pour avoir des points de vues différents.


Nous implémentons les mêmes heuristiques que celles utilisées par \dcs soit :
\begin{itemize}
	\item LPT rule
	\item SLACK
	\item LDM
...
Ceci nous permet de les comparer, au niveau Makespan obtenu et 
 le temps nécessaire à chacun d'eux pour donner leur réponse par rapport aux résultats obtenus par \dcs.
...
%\dcs utilisent des instances de taille 10, 50,100,500 et 1000 jobs à planifier sur 5, 10 et 25 machines %identiques.

%Ce protocole est reproduit. Mais d'autres paramètres peuvent être choisis pour voir le comportement de ces %algorithmes, au-delà des limites que se sont imposés \dcs pour comparer SLACK aux autres heuristiques. 
...
\bigskip
Après une présentation des différentes façons choisie d'obtenir des listes de temps de jobs synthétiques et réelles, nous aborderons une méthode permettant de transformer une liste de temps en une instance dont l'optimal est connu, pour finir sur la caractérisation des listes.  
...
\begin{itemize}
	...
	\item NON-UNIFORME
\end{itemize}
Dans le but d'élargir ce protocole et de comparer SLACK avec d'autres types de listes de temps, 3 autres distributions qui font parties de la familles des distributions 
  exponentielles (figure \ref{fig:taxonomieLoisStatistiques}) sont implémentées :
\begin{itemize}
	\item EXPONENTIELLE
	\item GAMMA
	\item BETA
