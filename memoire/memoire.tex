...
\subsection{Introduction} \label{subsec:etatDeLArtIntroduction}
...
Les centres de calculs sont dotés de parcs assez similaires, et maintenant, 
...
C'est un champ de la recherche opératoire, actif depuis plus d'un siècle, 
...
\subsection{Heuristiques}\label{subSec:Heuristiques}
...
Elles sont basées 
  ...
  sur le le problème bin-packing, 
  sur le problème de partitionnement de nombres, ou 
  reposent sur une stratégie gloutonne.
...
LPT Rule (Longest Time Processing) proposé par Graham \emph{et al.} \cite{graham1966bounds} est 
  un (algorithme \ref{algo:LPT} des algorithmes le plus repris dans la littérature. 
% -------------------------------------------------------
% LPT 
% -------------------------------------------------------
\begin{algorithm}[H]
...
\subsection{Programmation linéaire}\label{subSec:programmationLineaire}

\subsection{conclusion}\label{subSec:etatDeLArtConclusion}
