...
\caption{Le Supercomputer Fugaku (Fujitsu), du Centre pour la science
  des ordinateurs de RIKEN au Japon, vient de détrôner (www.top500.org
  parution de juin 2020) le Summit (IBM)de l’Oak Ridge National
  Laboratory aux États-Unis.
  Il compte 7,630,848 c\oe{}urs basés sur des processeurs ARM A64FX
  48C cadencés à 2.2GHz, pour 5,087,232 GB de ram.
  Il fonctionne sous RedHat.
}
\end{figure}
...
% RESULTAT OPTIMAL
% -------------------------------------------------------
\item $C_m^\star(J)$ Le makespan optimal, idéal, de l'ordonnancement d'un
...
% -------------------------------------------------------
\item $\Gamma(A)=\dfrac{C_m^A(J)}{C_m^\star(J)}$ Le ratio
  d'approximation atteint par l'algorithme $A$ au pire cas.
...
Ces algorithmes donnent des résultats, dont la borne minimale
$ \max_j\{P_j\} \le C_m^A(J)_{minimal} \le \dfrac{\sum_{j=1}^{n}
  P_j}{m}$ \todo{FCO: Trouver référence dans les publis} et la borne
maximale (au pire des cas $C_m^A(J)_{maximal}$ n'est pas maîtrise).
...
% LPT Rule
% -------------------------------------------------------
\paragraph{LPT rule (Graham \emph{et al.}, 1969)}
...
\paragraph{LPT-REV (Croce \emph{et al.}, 2018)}
...
COMBINE, améliore certains point de l'algorithme MULTIFIT en se basant
sur les principes suivants:
\begin{itemize}
\item Le résultat $M$ obtenu par LPT, est forcément supérieur ou égal
  à $M^\star$, soit $M>M^\star$.
  COMBINE utilise donc $M = C_m^{lpt}(J)$ comme borne supérieur $C_u$
  ...
  supérieure de départ $2\cdot A$ utilisé à l'origine par MULTIFIT.
...
\item Au lieu de prédéterminer un nombre d'itérations, COMBINE utilise
  ...
  COMBINE stope la recherche dès que $C_u - C_l \le \alpha \cdot A$.
  Les tests de COMBINE ont été effetués avec $\alpha = 0,005$
\end{itemize}
...
% SLACK
% -------------------------------------------------------
\paragraph{SLACK (Croce \emph{et al.}, 2018)}
...
\paragraph{PTAS (Hochbaum \emph{et al.})}
...
$M_1 = \{ 6,13,73 \}$ et  $M_2 = \{ 3,20,30,40 \}$ (voir Figure \ref{fig:LDM2M}).
